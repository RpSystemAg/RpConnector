...
$r=PRSTUDIO_UC_Performance_Budget::exceeded($d,['operations'=>1000,'duration'=>30]);ok($r['exceeded']===false&&$r['advisory']==='none'&&$r['blocking']===false,'equal boundary remains nonblocking telemetry');
$r=PRSTUDIO_UC_Performance_Budget::exceeded($d,['operations'=>1001]);ok($r['exceeded']===true&&$r['reasons']===['operations']&&$r['advisory']==='compact_or_checkpoint'&&$r['blocking']===false,'over boundary emits advisory telemetry without becoming an execution gate');
