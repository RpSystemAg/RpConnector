...
$raw=(string)file_get_contents(PRSTUDIO_UC_DIR.'capabilities/enterprise-capability-contracts.json');$doc=json_decode($raw,true);ok(is_array($doc)&&JSON_ERROR_NONE===json_last_error(),'enterprise contract source valid JSON');$contractIds=array_keys((array)$doc['contracts']);ok(array_values(array_intersect($ids,$contractIds))===$ids,'contract source contains the complete batch-1 contract set inside the unified catalog');
