...
BASE=Path('/mnt/data/prstudio_original_1700/PR-STUDIO-Unified-Suite-1.0.0')
...
with (ROOT/'ONE-GUARD-RUNTIME-CLASSIFICATION-1.0.0.ndjson').open('w') as f:
...
status={
 'suite_version':'1.0.0','reference_date':'2026-08-17','ONE_GUARD_STATUS':{'ANTI_CRASH':'PRESENT','OTHER_MUTATION_GUARDS':'ZERO'},
...
(ROOT/'ONE-GUARD-STATUS-1.0.0.json').write_text(json.dumps(status,ensure_ascii=False,sort_keys=True,indent=2)+'\n')
bench={
 'suite_version':'1.0.0','reference_date':'2026-08-17','evidence_policy':'measured where locally reproducible; structural where live authenticated browser/provider access is unavailable',
...
(ROOT/'ONE-GUARD-BENCHMARK-1.0.0.json').write_text(json.dumps(bench,ensure_ascii=False,sort_keys=True,indent=2)+'\n')
removed_files=[]
if BASE.exists():
