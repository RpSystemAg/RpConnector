...
export function actionToSteps(action, args = {}) {
  ...
  const direct = {
    ...
    playwright_list_pages: [{ type: "list_tabs", tabId: tabArg(args), activate: Boolean(args.activate) }],
    ...
    playwright_scroll: [{ ...s, type: "scroll", tabId: tabArg(args), x: Number(args.x || args.delta_x || 0), y: Number(args.y || args.delta_y || args.amount || 0), to: args.to || args.position || "", progressive: Boolean(args.progressive || args.to === "bottom") }],
    ...
    playwright_screenshot_page: [{ region: args.region || null, scale: args.scale ? Number(args.scale) : 0, type: "screenshot", tabId: tabArg(args), fullPage: Boolean(args.full_page), lazyLoad: args.lazy_load !== false, format: args.format || "auto", quality: Number(args.quality || 82), maxPixels: Number(args.max_pixels || args.maxPixels || 28000000) }],
