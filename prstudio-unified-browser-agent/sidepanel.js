...
$("pairButton").addEventListener("click", async () => { const code = $("pairCode").value.trim(); if (!code) return show("Inserisci il nuovo codice pairing.", true); show("Associazione in corso…", false); const result = await send("pair", { siteUrl: $("siteUrl").value.trim(), code, name: $("deviceName").value.trim() }); show(result?.ok ? "Associazione riuscita." : `Errore: ${errText(result)}`, !result?.ok); if (result?.ok) { await refreshRemote(); $("pairCode").value = ""; } });
