...
final class PRSTUDIO_UC_Bridge {
	...
	private static function task_response( array $task ): array {
		...
		if ( $executed && ! $completed_verified && empty( $error ) ) {
			$error = array( 'code'=>'browser_effect_unverified', 'message'=>'Il Browser Agent ha terminato il task, ma l’effetto richiesto non è stato verificato.', 'retryable'=>false );
		...
		$ok = ! $terminal || $completed_verified || $cancelled;
		return array(
			'ok'=>$ok,
			'terminal'=>$terminal,
			'provider'=>'prstudio_chrome_extension',
			...
			'next_action'=>$terminal ? null : 'browser_status',
