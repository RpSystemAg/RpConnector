...
final class PRSTUDIO_UC_Commerce_Engine {
    ...
    public static function product_update(array $args){
        ...
        if(isset($changes['seo'])&&is_array($changes['seo'])&&function_exists('update_post_meta')){
            ...
            foreach($changes['seo'] as $key=>$value){if(!isset($seo_map[$key]))return new WP_Error('prstudio_product_seo_field_unsupported','Unsupported SEO field.',array('status'=>400,'field'=>$key));update_post_meta($id,$seo_map[$key],is_scalar($value)?(string)$value:'');$changed[]='seo.'.$key;}
        }
