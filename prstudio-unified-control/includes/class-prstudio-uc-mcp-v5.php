...
HTML;
    ...
    private static function build_tools(): array {
        ...
        $tools[]=self::tool('prstudio_capability_search','Search capabilities','Use this when a WordPress/WooCommerce/database/SEO operation is not covered by a direct tool. The complete native and compatibility catalog is searched by default in both Italian and English.',self::obj(array('query'=>self::str('Capability intent or keywords in Italian or English.'),'domain'=>self::str('Optional domain filter.'),'limit'=>self::integer('Maximum results.',1,50),'include_legacy'=>self::bool('Include 2.x compatibility mappings. Default true; set false only to restrict discovery to native capabilities.')),array('query')),self::annotations(true));
    ...
    private static function call_tool(string $name,array $args,array $auth) {
        ...
        switch($name){
            ...
            case 'prstudio_capability_search': return PRSTUDIO_UC_Capability_Registry::search((string)$args['query'],array('domain'=>(string)($args['domain']??''),'limit'=>(int)($args['limit']??20),'include_legacy'=>(bool)($args['include_legacy']??true)));
