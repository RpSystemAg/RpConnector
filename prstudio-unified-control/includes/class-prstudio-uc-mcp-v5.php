...
HTML;
    ...
    private static function operator_instructions(): string {
        ...
        return 'PR STUDIO 17.0. You are this site\'s executor: make the change, verify it, report it. '
            ...
            . 'Fast paths: snapshot=browser_snapshot; screenshot-only=browser_screenshot; open=browser_open; deterministic browser sequence=browser_batch; direct page JS=browser_javascript_exec; navigate=browser_navigate; click=browser_click; fill=browser_fill; tabs=browser_tabs. browser_open claims the new background tab in the existing Chrome window for the lane before navigation and lane ownership persists across later tasks: never re-adopt an Agent-created tab. browser_adopt_tabs is only for an existing user tab explicitly selected for the lane. '
    ...
    private static function build_tools(): array {
        ...
        $tools[]=self::tool('serp_configure','Configure SerpBear','Configure the SerpBear base URL, domain and GSC property; API key is written only to the encrypted Secrets Vault and never returned.',self::obj(array('base_url'=>self::str('SerpBear base URL.'),'api_key'=>self::str('SerpBear API key.'),'domain'=>self::str('Tracked domain.'),'site_url'=>self::str('GSC property URL.'),'allow_private_host'=>self::bool('Allow an explicitly trusted private/local SerpBear host.')),array('base_url','api_key'),false),self::annotations(false,false,true,true));
        $tools[]=self::tool('serp_status','SERP watcher status','Inspect SerpBear configuration, durable keyword watches and scheduler readiness without exposing API secrets.',self::obj(),self::annotations(true,false,true,true));
        $tools[]=self::tool('serp_keywords','SerpBear keywords','Read the configured SerpBear keyword inventory and exact latest SERP positions.',self::obj(array('domain'=>self::str('Optional SerpBear domain override.')),array(),false),self::annotations(true,false,true,true));
        $tools[]=self::tool('serp_watch_create_all','Watch every SerpBear keyword','Create sharded durable daily watches for every keyword currently configured in SerpBear. Each keyword records first-page and #1 milestones independently.',self::obj(array('domain'=>self::str('Optional SerpBear domain override.'),'site_url'=>self::str('GSC property URL.'),'target_position'=>self::integer('Target exact SERP position.',1,100),'first_page_position'=>self::integer('First-page milestone threshold.',1,100),'required_consecutive'=>self::integer('Consecutive confirmations.',1,14),'interval_hours'=>self::integer('Check interval in hours.',1,168),'refresh_serpbear_before_check'=>self::bool('Refresh all SerpBear ranks before a watch check.')),array(),false),self::annotations(false,false,true,true));
        $tools[]=self::tool('serp_watch_create','Create keyword rank watch','Create a durable daily rank watch. SerpBear exact rank is the completion authority; GSC is supporting first-party evidence.',self::obj(array('keywords'=>array('type'=>'array','minItems'=>1,'maxItems'=>500,'items'=>self::str('Exact keyword.')),'domain'=>self::str('SerpBear domain.'),'site_url'=>self::str('GSC property URL.'),'target_position'=>self::integer('Target exact SERP position.',1,100),'first_page_position'=>self::integer('First-page milestone threshold.',1,100),'required_consecutive'=>self::integer('Consecutive checks required before success.',1,30),'interval_hours'=>self::integer('Check interval in hours.',1,168),'refresh_serpbear_before_check'=>self::bool('Run documented SerpBear cron refresh before each check.')),array('keywords'),false),self::annotations(false,false,true,true));
        $tools[]=self::tool('serp_watch_run','Run keyword rank watch','Run one bounded watch check now and persist SerpBear/GSC evidence.',self::obj(array('watch_id'=>self::str('Watch ID.')),array('watch_id'),false),self::annotations(false,false,true,true));
        $tools[]=self::tool('serp_watch_list','List keyword rank watches','List durable keyword watches and milestones.',self::obj(),self::annotations(true,false,true,true));
        $tools[]=self::tool('serp_watch_get','Get keyword rank watch','Read one watch, keyword milestones and recent history.',self::obj(array('watch_id'=>self::str('Watch ID.')),array('watch_id'),false),self::annotations(true,false,true,true));
        $tools[]=self::tool('serp_watch_cancel','Cancel keyword rank watch','Cancel future checks without deleting history or evidence.',self::obj(array('watch_id'=>self::str('Watch ID.'),'reason'=>self::str('Cancellation reason.')),array('watch_id'),false),self::annotations(false,false,true,true));
        $tools[]=self::tool('serp_refresh_all','Refresh all SerpBear keywords','Trigger the documented SerpBear POST /api/cron refresh. Requires explicit refresh_all=true.',self::obj(array('refresh_all'=>self::bool('Must be true.')),array('refresh_all'),false),self::annotations(false,false,false,true));
        ...
        $tools[]=self::tool('procedural_skill_search','Search learned procedures','Search verified procedural skills before retrying a known task.',self::obj(array('query'=>self::str('Task/capability/action intent.'),'kind'=>self::str('Optional capability or browser kind.'),'limit'=>self::integer('Maximum matches.',1,50)),array('query'),false),self::annotations(true));
        ...
        $tools[]=self::tool('browser_scroll','Browser scroll','Use this to scroll the selected tab like a human. Can scroll progressively to bottom.',self::obj(array_merge($tab,array('x'=>array('type'=>'number'),'y'=>array('type'=>'number'),'to'=>self::str('top, bottom or position.'),'progressive'=>self::bool('Progressive human-like scroll.'))),array(),false),self::annotations(false,false,false,true));
        $tools[]=self::tool('browser_screenshot','Browser screenshot','Capture verified visual evidence. Storage is preflighted, oversized full-page captures are bounded instead of retried in loops, and large captures may use JPEG automatically.',self::obj(array_merge($tab,array('full_page'=>self::bool('Capture full page when safely bounded.'),'format'=>self::str('auto, png or jpeg.',array('enum'=>array('auto','png','jpeg'))),'quality'=>self::integer('JPEG quality when used.',35,92),'max_pixels'=>self::integer('Hard output pixel budget.',1000000,28000000),'ocr'=>self::bool('Extract visible pixel text only when needed.'),'ocr_language'=>self::str('OCR languages.'))),array(),false),self::annotations(true,false,true,true));
        ...
        $tools[]=self::tool('browser_javascript_exec','Execute JavaScript in controlled tab','Execute deterministic JavaScript directly in the owned browser tab when DOM/API code is already the shortest path; no locator discovery round-trip.',self::obj(array('script'=>self::str('JavaScript source to execute.'),'tab_id'=>self::integer('Optional owned tab id.')),array('script')),self::annotations(false,true,false,true));
    ...
    private static function call_tool(string $name,array $args,array $auth) {
        ...
        switch($name){
            ...
            case 'serp_configure': return PRSTUDIO_UC_Serp_Watch::configure($args);
            case 'serp_status': return PRSTUDIO_UC_Serp_Watch::status($args);
            case 'serp_keywords': return PRSTUDIO_UC_Serp_Watch::keywords($args);
            case 'serp_watch_create_all': return PRSTUDIO_UC_Serp_Watch::watch_create_all($args);
            case 'serp_watch_create': return PRSTUDIO_UC_Serp_Watch::watch_create($args);
            case 'serp_watch_run': return PRSTUDIO_UC_Serp_Watch::watch_run($args);
            case 'serp_watch_list': return PRSTUDIO_UC_Serp_Watch::watch_list($args);
            case 'serp_watch_get': return PRSTUDIO_UC_Serp_Watch::watch_status($args);
            case 'serp_watch_cancel': return PRSTUDIO_UC_Serp_Watch::watch_cancel($args);
            case 'serp_refresh_all': return PRSTUDIO_UC_Serp_Watch::refresh($args);
            ...
            case 'browser_javascript_exec': return self::browser_dispatch('playwright_evaluate',array('script'=>(string)($args['script']??''),'tab_id'=>$args['tab_id']??null));
