...
final class PRSTUDIO_UC_Execution_Gateway {
    ...
    private static function invoke(array $cap,array $args){
        ...
        [$class,$method]=explode('::',$executor,2);if(!class_exists($class)||!is_callable(array($class,$method)))return self::error('capability_executor_missing','Capability executor is unavailable.',500,array('capability'=>$cap['id']??'','executor'=>$executor));
        try{$rm=new ReflectionMethod($class,$method);return $rm->getNumberOfParameters()>=2?$class::$method($args,$cap):$class::$method($args);}catch(Throwable $e){return self::error('capability_execution_exception','Capability execution failed safely.',500,array('capability'=>$cap['id']??'','exception_class'=>get_class($e)),true);}
    }
    private static function terminal(array $job):bool{return PRSTUDIO_UC_Store::terminal_job_state((string)($job['status']??''));}
    private static function mission_event(string $mission_id,string $event,array $payload=array()):void{
        ...
        try{
            $state=PRSTUDIO_UC_Memory::mission($mission_id);if(!is_array($state))$state=array();
            $state['mission_id']=$mission_id;$state['status']=(string)($payload['status']??$state['status']??'running');
            $state['resume_point']=(string)($payload['resume_point']??$state['resume_point']??'');
            $state['last_event']=$event;$state['last_job_id']=(string)($payload['job_id']??$state['last_job_id']??'');
    ...
    private static function qcount(): int { global $wpdb; return isset($wpdb)&&is_object($wpdb)?(int)($wpdb->num_queries??0):0; }
    private static function sql_time_from(int $saved_index): ?float {
        ...
        $rows=array_slice($wpdb->queries,$saved_index);$seconds=0.0;
        foreach($rows as $row){if(is_array($row)&&isset($row[1])&&is_numeric($row[1]))$seconds+=(float)$row[1];}
        return round($seconds*1000,3);
    }
    private static function saved_query_count(): int { global $wpdb; return isset($wpdb)&&is_object($wpdb)&&is_array($wpdb->queries??null)?count($wpdb->queries):0; }
    ...
    private static function execute_inline_cap(array $cap,array $args,array $request,array $lane,float $started){
        $cap_id=(string)$cap['id'];$is_write=empty($cap['read_only']);$dry=!empty($request['dry_run']);
        if($dry&&$is_write)return array('ok'=>true,'status'=>'completed','fast_path'=>true,'dry_run'=>true,'result'=>array('would_execute'=>$cap_id,'execution_contract'=>array_intersect_key($cap,array_flip(array('execution_class','preferred_executor','estimated_work','minimal_verification')))));
        $q0=self::qcount();$sq0=self::saved_query_count();$invoke_started=microtime(true);
        $invoke_args=$args;$invoke_args['_client_id']=(string)($request['_owner_client_id']??'');
        ...
        if($is_write&&class_exists('PRSTUDIO_UC_Pre_Mutation_Safety')){
            ...
            if('deferred'!==$scope){$gate=PRSTUDIO_UC_Pre_Mutation_Safety::before_commit($scope,$cap_id,$invoke_args);if(is_wp_error($gate))return $gate;}
        }
        $result=self::invoke($cap,$invoke_args);$invoke_ms=round((microtime(true)-$invoke_started)*1000,3);
        if(is_wp_error($result))return $result;
        $verification=self::minimal_verify($cap,$result);$total_ms=round((microtime(true)-$started)*1000,3);$sql_ms=self::sql_time_from($sq0);
    ...
    public static function execute(array $request){
        ...
        $impact=PRSTUDIO_UC_Impact_Engine::analyze($cap,$args);$risk=PRSTUDIO_UC_Risk_Engine_V3::evaluate($cap,$args,$impact,$dry,array());$plan=PRSTUDIO_UC_Planner_V3::plan($cap,$args,$impact);
        $key_in=(string)($request['idempotency_key']??'');$key_seed=''!==$key_in?$key_in:('request:'.$request_id);$key=hash('sha256','capability|'.PRSTUDIO_UC_Memory::site_identity()['key'].'|'.$cap_id.'|'.$key_seed);
        // Preparation is intentionally allowed before the site mutation gate.
        // Legacy capabilities defer to their concrete REST/direct executor so the
        // guard is not duplicated and can run after executor preconditions.
        ...
        $job=PRSTUDIO_UC_Store::create_job('execute '.$cap_id,(string)$cap['domain'],array('arguments'=>$args,'dry_run'=>$dry,'execution_mode'=>(string)($request['execution_mode']??'sync'),'lane_id'=>is_array($lane)?(string)($lane['lane_id']??''):''),$plan,$key,hash('sha256',PRSTUDIO_UC_Idempotency::canonical_json($plan)),array('mission_id'=>(string)($request['mission_id']??''),'owner_client_id'=>$owner_client,'request_id'=>$request_id));
        $job_id=(string)($job['job_uuid']??'');$idempotent_replay=!empty($job['idempotent_replay']);if(''===$job_id)return self::error('job_create_failed','Unable to persist job.',500,array(),true);
        ...
        $mission_id=(string)($request['mission_id']??'');self::mission_event($mission_id,'job.planned',array('job_id'=>$job_id,'capability'=>$cap_id,'status'=>'running','resume_point'=>'execute:'.$cap_id));$job=PRSTUDIO_UC_Store::get_job($job_id)?:$job;if($idempotent_replay||self::terminal($job)){self::mission_event($mission_id,'job.reused',array('job_id'=>$job_id,'capability'=>$cap_id,'completed_steps'=>array($cap_id),'resume_point'=>'after:'.$cap_id));$job['idempotent_replay']=true;return array('ok'=>true,'job'=>$job,'idempotent_replay'=>true);}
        if($dry && $is_write){PRSTUDIO_UC_Store::set_job_state($job_id,'COMPLETED',array('result'=>array('dry_run'=>true,'would_execute'=>$cap_id,'impact'=>$impact,'risk'=>$risk,'snapshot_policy'=>$cap['snapshot_policy'],'verification_policy'=>$cap['verification_policy'],'plan'=>$plan),'verification'=>array('ok'=>true,'verifier'=>'dry_run'),'progress'=>100));self::mission_event($mission_id,'job.dry_run_completed',array('job_id'=>$job_id,'capability'=>$cap_id,'completed_steps'=>array('dry_run:'.$cap_id),'resume_point'=>'execute:'.$cap_id));return array('ok'=>true,'job'=>PRSTUDIO_UC_Store::get_job($job_id),'dry_run'=>true);}
        $budget=PRSTUDIO_UC_Performance_Budget::normalize(is_array($request['budget']??null)?$request['budget']:array());$budget_check=PRSTUDIO_UC_Performance_Budget::exceeded($budget,array('duration'=>microtime(true)-$started,'operations'=>0,'network_requests'=>0,'retries'=>0,'memory_mb'=>memory_get_usage(true)/1048576));$budget_check['advisory_only']=true;
        PRSTUDIO_UC_Store::set_job_state($job_id,'RUNNING',array('progress'=>15));$snapshot=array('ok'=>true,'skipped'=>true,'reason'=>'anti_crash_is_only_pre_mutation_guard');
        $invoke_args=$args;$invoke_args['_client_id']=$owner_client;if(''!==$lane_token)$invoke_args['lane_token']=$lane_token;if(!empty($cap['browser_required'])){$invoke_args['_prstudio_job_uuid']=$job_id;$invoke_args['sync_wait_seconds']=0;}
        if($is_write&&!$dry&&class_exists('PRSTUDIO_UC_Pre_Mutation_Safety')&&'deferred'!==$mutation_scope){$anti_args=$args;if(isset($request['work_id']))$anti_args['work_id']=$request['work_id'];$anti=PRSTUDIO_UC_Pre_Mutation_Safety::before_commit($mutation_scope,$cap_id,$anti_args);if(is_wp_error($anti))return $anti;}
        $result=self::invoke($cap,$invoke_args);if(is_wp_error($result)){if(class_exists('PRSTUDIO_UC_Procedural_Skills')){try{PRSTUDIO_UC_Procedural_Skills::observe_failure('capability',$cap_id,$args,array('code'=>$result->get_error_code(),'message'=>$result->get_error_message()));}catch(Throwable $ignored){}}PRSTUDIO_UC_Store::set_job_state($job_id,'TECHNICAL_ERROR',array('error'=>array('code'=>$result->get_error_code(),'message'=>$result->get_error_message()),'progress'=>100));self::mission_event($mission_id,'job.technical_error',array('job_id'=>$job_id,'capability'=>$cap_id,'status'=>'technical_error','retry_path'=>'retry_or_alternate:'.$cap_id,'failed_paths'=>array(array('capability'=>$cap_id,'code'=>$result->get_error_code(),'gmt'=>gmdate('c')))));return $result;}
        $browser_wait=is_array($result)&&in_array(strtolower((string)($result['status']??$result['task']['status']??'')),array('queued','leased','running','waiting_for_browser'),true);if($browser_wait){$task_id=(string)($result['task_id']??$result['task']['task_uuid']??'');PRSTUDIO_UC_Store::set_job_state($job_id,'WAITING_FOR_BROWSER',array('checkpoint'=>array('browser_task_id'=>$task_id,'browser_result'=>$result),'result'=>$result,'progress'=>50));self::mission_event($mission_id,'job.waiting_browser',array('job_id'=>$job_id,'capability'=>$cap_id,'status'=>'waiting','resume_point'=>'browser_task:'.$task_id));return array('ok'=>true,'job'=>PRSTUDIO_UC_Store::get_job($job_id),'waiting_for_browser'=>true);}
        // PR STUDIO ONE-GUARD INVARIANT: verification is evidence only. The job
        // remains RUNNING while evidence is observed; there is no verifier state/gate.
        try{
